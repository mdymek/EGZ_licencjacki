...
\begin{document}
    ...
    \begin{definition}
	Układem sekwencyjnym (ang. sequential logic circuits) nazywamy układ, w którym poziomy sygnałów wyjściowych zależą nie tylko od aktualnego stanu poziomów sygnałów na jego wejściu ale również od stanu poziomów,
	które występowały uprzednio. Oznacza to, że układy te zawierają elementy pamiętające i nazywane są czasem układami kombinacyjnymi z pamięcią.
    \end{definition}
	...
	\noindent Przykładowy schemat blokowy przeżutnika (w tym przypadku część odpowiedzialna za przerzutnik RS została zbudowana na bramkach NAND - ta wersja nazywa się SR,
    ...
    \end{definition}
       ...
       \begin{definition}
    	\textbf{Przerzutnik JK} - podobnie jak przerzutnik D jest synchroniczny oraz powstał poprzez modyfikację przerzutnik RS (SR). W tej wersji pozbyto się kłopotliwego stanu, gdy na oba wejścia przerzutnika RS podana zostanie 1.
    	Przerzutnik JK posiada 3 wejścia: J, K oraz Clock. Porównując RS do JK: odpowienikiem wejścia S jest wejście J, a odpowiednikiem wejścia R jest wejście K.
	...
	\end{tabular}
	\end{table}
	Jak widać w przypadku podania na oba wejścia (J i K) 1, przeżutnik zmienia stan wyjścia na odwrotny w stosunku do tego, który był przed podaniem sygnału zegarowego. \\
	\noindent Przykładowy schemat blokowy przeżutnika (na bramkach NAND): \\
    ...
    \begin{definition}
    Minimalizacja z wykorzystaniem algebry Boole'a polega na zpisaniu funkcji w postaci sumy stanów wejść dla których funkcja przyjmuje wratość 1, a następnie wykorzystując prawa algebry Boole'a uproszczenie jej.
    \end{definition}
    
    \subsection{Minimalizacja z wykorzystaniem tablic Karnaugha}
    \begin{definition}
    Minimalizacja z wykorzystaniem tablic Karnaugha polega na podziale argumentów funkcji na dwie grupy, a następnie zapisaniu tej funkcji w postaci tabeli,
    gdzie nagłówkami wierszy są poszczególne kombinacje argumentów z pierwszej grupy, nagłówkami kolumn kombinacje argumentów z drugiej grupy,
    a wartościami w komórkach wartości naszej funkcji dla poszczególnych argumentów. \\
    W tak przygotowanej tabeli szukamy takich obszarów, aby prostokontnym zaznaczeniem możnabyło objąć 1 w ilości będącą potęgą 2 (1, 2, 4, 8, ...). Obszarami tymi musimy pokryć wszystkie 1.
    Obszary moga na siebie zachodzić.
    Ponadto tablicę tą możemy traktować jako tablicę cykliczną (początek i koniec danego wiersza lub kolumny łączą się ze sobą), co często ułatwia zaznaczanie obszarów. \\
    Gdy mamy już zaznaczone obszary, dla każdego z nich patrzymy od jakich zmiennych zależy, tzn. jeżeli dana zmienna zmienia się dla tego obszaru, oznacza to, że nie zależy on od tej zmiennej i możemy ją wykluczyć.
    Następnie zapisujemy dane obszary jako funkcje iloczynu niewykluczonych zmiennych, dla których obszar przyjmuje wartość 1 - czyli piszemy iloczyn wszystkich niewykluczonych zmiennych, a dodatkowo te zmienne,
    które na tym obszarze miały wartość 0 negujemy. \\
    Ostatecznie nasza zminimalizowana funkcja będzie miała postać sumy wszystkich funkcji dla wszystkich zaznaczonych obszarów wyznaczonych w poprzednim kroku.
    \end{definition}
