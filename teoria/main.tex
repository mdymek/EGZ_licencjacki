...
\begin{document}
    ...
    \section{Algorytmy wyszukiwania najkrótszej ścieżki (Dijkstry oraz Bellmana-Forda).}
        ...
        W algorytmach wyszukiwania najkrótszej ścieżki używamy procedury \italic{relax(u, v, w)}, 
        sprawdzającej, czy ścieżka z $u$ do $w$ przez $v$ jest krótsza od aktualnie najkrótszej 
        ścieżki.

            \[\]
        \textit{relax(u, v, w)}:
            \vskip Jeśli $D[u, w] = D[u, v] + D[v, w]$ to:
            \begin{enumerate}
                \item $D[u, w] = D[u, v] + D[v, w]$
                \item $P[u, w] = v$
            \end{enumerate}

        \subsection{Algorytm Dijkstry}
        
            Algorytm Dijkstry pozwala na wyliczenie w grafie $G = \{V, E\}$ najkrótszej ścieżki z wierzchołka
            startowego $s$ do wszystkich pozostałych wierzchołków. $|V| = v; |E| = e$

            \subsubsection{Algorytm}
            
            Posługujemy się słownikiem $P = \{\}$, przypisującego każdemu wierzchołkowi
            ...
            \[\]
            \textit{relax(u, v, w)}:
                \vskip Jeśli $D[w] = D[v] + e_{v \rightarrow w}$ to:
                \begin{enumerate}
                    \item $D[w] = D[v] + e_{v \rightarrow w}$
                    \item $P[w] = v$
                \end{enumerate}
        ...
        \subsection{Algorytm Bellmana-Forda}
            ...
            \vskip Dla każdej krawędzi $(u, v) \in E$ wywołaj \textit{relax(u, v)}
            \[\]
            Dodatkowo można wykonać sprawdzenie, czy w grafie nie ma cykli o wadze ujemnej:

            Dla każdej krawędzi $(u, v) \in E$: jeśli $D[v] > D[u] + e_{u \rightarrow v}$, to
            istnieje cykl o wadze negatywnej.
            \subsubsection{Złożoność}
            ...
            \begin{itemize}
                ...
                \item Pozwala decydować, czy graf ma cykle negatywne
        ...
        \subsection{Algorytm Floyda-Warshalla}
            ...
            Posługujemy się słownikiem $P = \{\}$, przypisującego parze $(u, w)$ rodzica dla $u$,
            na ścieżce od $u$ do $w$; słownikiem $D$ przypisującego parze $(u, w)$ długość 
            ...
            Dla każdej trójki $(u, v, w) : u, w, v \in V$ wywołaj \textit{relax(u, v, w)}
            ...
            \begin{itemize}
                ...
                \item Złożoność jest niezależna od gęstości grafu: nieoptymalny dla grafów 
                    rzadkich
            \end{itemize}
