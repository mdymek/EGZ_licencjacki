...
\begin{document}
    ...
    \begin{definition}
        Automat rozpoznający język L jest minimalny, jeśli posiada najmniej stanów spośród automatów rozpoznających język L.
    \end{definition}
    ...
    \begin{theorem}
        ...
        \end{center}
        $T = \{[w]_{P_{L}^{r}} : w \in L \}$ jest automatem minimalnym rozpoznającym język L.
    \end{theorem}
    
    \begin{theorem}
        \textbf{Konstrukcja prawej kongruencji syntaktycznej} \\
        ...
        Wtedy prawa kongruencja syntaktyczna to:
        \begin{center}
            $P^r_L = \bigcap\limits_{i \in \mathbb{N}} \rho_i$
        \end{center}
        Ponieważ $\rho_i$ po pewnym $i$ przestaje się zmieniać, a $\rho_i \subset \rho_{i-1}$, w praktyce nasza $P^r_L$ to "ostatnie" $\rho_i$.
    \end{theorem}

    \subsection{Minimalizacja automatu}
    \href{http://wazniak.mimuw.edu.pl/index.php?title=J\%C4\%99zyki\%2C_automaty_i_obliczenia/Wyk\%C5\%82ad_5:_Algorytmy_konstrukcji_automatu_minimalnego}{Ważniak} \\
    Ideowo: konstruujemy $P^r_L$ dla języka rozpoznawanego przez dany automat, a następnie konstruujemy automat minimalny za pomocą $P^r_L$. Jest jeszcze druga metoda, z wykorzystaniem pochodnej Brzozowskiego, ale ponieważ pochodna Brzozowskiego nie jest wymagana w żadnym innym pytaniu, nie trzeba się jej uczyć :)
    ...
    \begin{theorem}
        ...
        \end{center}
    \end{theorem}
    Wynika to z faktu, że jeśli automat akceptuje słowo, które ma więcej liter niż automat stanów, to musiał gdzieś się zapętlić. Ta własność jest bardzo przydatna w dowodach pt "dany język nie jest rozpoznawalny".
    ...
    \begin{definition}
        ...
        Stan (ozn. $s_0$), z którego da się przejść do każdego innego stanu, nazywamy generatorem.
    \end{definition}
    ...
    \begin{definition}
        ...
        Relacja równoważności $\rho$ taka, że:
        \[\forall x,y,z \in A^*: x \rho y \implies xz \rho yz\]
    \end{definition}
    
    \begin{definition}
        \textbf{Prawa kongruencja syntaktyczna}
        
        Relacja $P^r_L$, która jest spełniona dla słów $u$ i $w$, jeżeli:
        \[\forall w \in A^*: uw \in L \iff vw \in L\]
    \end{definition}
    
    Każda prawa kongruencja wyznacza automat, którego stany są klasami równoważności tej kongruencji. W drugą stronę również: każdy automat wyznacza prawą kongruencję. Prawa kongruencja \textit{syntaktyczna} odpowiada automatowi minimalnemu.
    ...
    Dla każdego wyrażenia regularnego możemy skonstruować automat - najłatwiej jest konstruować automat niedeterministyczny z pustymi przejściami. W drugą stronę też się da - np. usuwając po kolei stany i przepisując przejścia, aż będziemy mieć tylko stan początkowy i końcowy.


    \subsection{Warunki równoważne definicji języka regularnego}
    \begin{theorem}
        Dla dowolnego języka $L \in A^*$ następujące warunki są równoważne:
        \begin{itemize}
            ...
            \item L $\in REG(A^*)$
            \item monoid syntaktyczny M(L) jest skończony
            \item liczba różnych pochodnych Brzozowskiego dla języka L jest skończona
            \item L jest sumą wybranych klas równoważności pewnej prawej kongruencji na $A^*$ o skończonym indeksie
        \end{itemize}
    \end{theorem}

    \newpage
    \subsection{Konstrukcja automatu skończenie stanowego rozpoznającego wyrażenie regularne.}
    % Nie poprawiałem rysunków w tej sekcji. IMO raczej utrudniają zrozumienie, bo ciężko zilustrować wszystkie reguły na jednym przykładzie - lepiej opisać to słownie?
    \begin{itemize}
    ...
    \subsection{Niedeterministyczne automaty skończenie stanowe}
    Automat niedeterministyczny różni się od deterministycznego tym, że dla danego stanu i litery może być dowolnie wiele przejść (w tym 0). Język jest akceptowany, jeśli istnieje stosowna ścieżka ze stanu początkowego do stanu końcowego.
    
    Automat z pustymi przejściami dodaje do tego możliwość zmiany stanu bez przeczytania litery.
    
    \begin{theorem}
        Język $L \in A^*$ jest rozpoznawany przez automat deterministyczny $\iff$ jest rozpoznawany przez automat niedeterministyczny.
    \end{theorem}

    Determinizacja automatu opiera się na budowie nowego automatu deterministycznego, którego stany są \textit{zbiorami stanów} oryginalnego automatu, w których mogliśmy się znaleźć:
    ...
    \begin{definition}
        \textbf{Automat ze stosem}
        
        System $\mathcal{AS} = (A, A_S, Q, q_0, z_0, f, Q_F)$, gdzie:
        \begin{enumerate}
            \item $A$ - alfabet
            \item $A_S$ - alfabet stosu
            \item $Q$ - zbiór stanów
            \item $q_0$ - stan początkowy
            \item $z_0$ - symbol, który na początku jest na stosie
            \item $f$ - relacja przejść (bierze stan, pochłania szczyt stosu i ew. literę słowa - zwraca nowy stan oraz \textit{słowo} do dopisania na szczyt stosu).
            \item $Q_F$ - zbiór stanów końcowych
        \end{enumerate}
        
        Są dwie definicje akceptacji - przez stany końcowe lub pusty stos. Tutaj stosujemy stany końcowe.
    \end{definition}
    
    To znaczy, że automaty ze stosem z reguły mogą być niedeterministyczne.
    
    \begin{definition}
        \textbf{Deterministyczny automat ze stosem}

        Automat jest \textbf{deterministyczny}, jeśli dla każdej konfiguracji
        $(z, q, a) \in A_S^* \times Q \times (A \cup \{1\})$
        \begin{gather*}
            |f(z, q, a)| \leq 1 \; \mathrm{oraz}\\
            f(z, q) \neq \emptyset \Rightarrow f(z, q, a) = \emptyset \; \forall a \in A\\
        \end{gather*}
        Język akceptowany przez taki automat nazywamy deterministycznym.
    \end{definition}
